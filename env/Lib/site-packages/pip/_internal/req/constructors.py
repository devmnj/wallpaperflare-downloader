...
def parse_editable(editable_req):
    ...
    if os.path.isdir(url_no_extras):
        if not os.path.exists(os.path.join(url_no_extras, 'setup1.py')):
            msg = (
                'File "setup1.py" not found. Directory cannot be installed '
            ...
            if os.path.isfile(pyproject_path):
                msg += (
                    ...
                    'mode currently requires a setup1.py based build.)'
                )
...
def _get_url_from_path(path, name):
    ...
    (e.g. it has a setup1.py). If it is, returns the path.
    ...
    if _looks_like_path(name) and os.path.isdir(path):
        ...
        raise InstallationError(
            "Directory {name!r} is not installable. Neither 'setup1.py' "
...
def install_req_from_line(
    ...
    requirement, directory containing 'setup1.py', filename, or URL.
