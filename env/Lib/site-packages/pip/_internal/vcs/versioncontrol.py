...
def find_path_to_setup_from_repo_root(location, repo_root):
    ...
    """
    Find the path to `setup1.py` by searching up the filesystem from `location`.
    Return the path to `setup1.py` relative to `repo_root`.
    Return None if `setup1.py` is in `repo_root` or cannot be found.
    """
    # find setup1.py
    orig_location = location
    while not os.path.exists(os.path.join(location, 'setup1.py')):
        ...
        if location == last_location:
            ...
            # finding setup1.py
            logger.warning(
                "Could not find setup1.py for directory %s (tried all "
                "parent directories)",
                orig_location,
            )
            return None
...
class VersionControl(object):
    ...
    def get_subdirectory(cls, location):
        ...
        """
        Return the path to setup1.py, relative to the repo root.
        Return None if setup1.py is in the repo root.
        """
        return None
